  ...
  <a-card :bordered="false">
    ...
    <a-col style="height: calc(100vh - 360px)">
      <vxe-table
        ...
        align="center"
      >
        <vxe-column field="equipmentName" :title="$t('basic.设备名称')"></vxe-column>
        <vxe-column field="equipmentNo" :title="$t('basic.设备编号')"></vxe-column>
        <vxe-column field="nickName" :title="$t('basic.设备昵称')"></vxe-column>
        <vxe-column field="assetNo" :title="$t('basic.固定资产号')"></vxe-column>
        <vxe-column field="deptName" :title="$t('basic.所属部门')"> </vxe-column>
        <vxe-column
          field="workingProcedureName"
          :title="$t('basic.所属工序')"
        ></vxe-column>
        <vxe-column field="detectionType" :title="$t('basic.检测类别')">
          <template v-slot="{ row }">
            {{
              row.detectionType == "0"
                ? "清洗液浓度"
...
import { equipmentPage, equipmentDelete } from "@/apis/basic";
...
//将子组件获取到的对象返回
const searchQuery = (formValue: FormStateTs) => {
...
const onDelete = (row: Object) => {
  equipmentDelete({ equipmentId: row.equipmentId }).then((res) => {
    setMessageTips(res);
    init("initial");
  });
};
